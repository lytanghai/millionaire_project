...
const url = backend_url + "/test/experiment"
...
// Send request to backend /experiment
async function sendRequest() {
    try {
        ...
        const queryObj = {}
        queryParams.value.forEach(q => {
            if (q.key && q.value) queryObj[q.key] = q.value
        })
        ...
        if (payload.value) {
            try {
                bodyObj = JSON.parse(payload.value)
            } catch {
        ...
        apiResponse.value = res.data.data || res.data
    } catch (err) {
